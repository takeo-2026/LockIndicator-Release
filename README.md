...
> インジケータは各ロックキーが **ON のときだけ** 表示されます。  
> OFF のときは何も表示されず、作業の邪魔になりません。  
 
<img src="fig01Indicator.png" width="800">

### **表示位置を選択する設定ウィンドウ**  
チェックボックスで、左上 / 右上 / 左下 / 右下 の表示位置を切り替えられます。
> 四隅すべてに表示すると、いかにも邪魔になりそうに思われますが、  
> 実際にはロックキーが **ON のときだけ** 表示されるため、普段は何も出ません。  
> そのため、四隅に配置しておくと、どの画面で作業していても **視線をずらさずに状態を確認できて** とても便利です。  
